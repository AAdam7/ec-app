...
export function ProductListItem() {
    ...
    return (
      <div class={styles.deliveryBox}>
        <ul>
          ...
          <li class={styles.isMobile}><TiTick size={24} color="#000000" />{appText.freeDelivery}</li>
          <li class={styles.isMobile}><TiTick size={24} color="#000000" />{appText.payPalCredit}</li>
        </ul>
      </div>
    );
  };
  ...
  const handleDiscount = (e) => {
	...
	const addRecommendedTag = () => {
		return [styles.productListItemWrapper, styles.productListItemWrapperShadow,].join(" ")
	}

	const isNormalStock = (e, someVar) => {
		return  e.stock > 50 ? someVar? `${styles.normalStock}` : `${styles.text}` : `${styles.lowStock}`
	}

	const PriceStatusBar = ({product}) => {
    return (
      <div>
        <div class={styles.originalPrice}><h3 style={`color:` + styles.rrp}>RRP £{product.price.toFixed(2)}</h3></div>
      ...
      <div>
        ...
        <div class={styles.stars}>
          <AiFillStar color="#F2C94C" size={24}/>
					...
					<AiFillStar color="#F2C94C" size={24}/>
					<AiFillStar color="#F2C94C" size={24}/>
					<AiFillStar color="#F2C94C" size={24}/> {Math.floor(Math.random() * 1000)} {appText.reviews}
        </div>
      </div>
    );
  };
  ...
  return (
    <For each={dataProducts()} fallback={<p>{appText.loading}</p>}>
      {(product) => (
        <div
          class={isRecommendedTag(product)? addRecommendedTag() : styles.productListItemWrapper}>
          {product.rating > 4.5 && (<div class={styles.labelRecommended}>{appText.eclipseRecommended}</div>)}
          <div class={styles.productListItem}>
					<div class={styles.isDesktop}><TitleReview product={product} /></div>
            <figure>
              <img src={product.images[0]} alt={product.title} />
            </figure>
          </div>
          <div class={styles.productListItem}>
						<div class={styles.isMobile}><TitleReview product={product} /></div>
